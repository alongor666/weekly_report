...
本文件定义了16个核心KPI的计算公式、业务逻辑和显示规则

## 1. 核心原则

- **数据驱动**: 所有KPI均源自文件夹中的基础数据。
- **一致性**: 命名、单位和计算逻辑在整个文件夹保持统一。
...
| KPI名称 | 计算公式 | 业务说明 |
| :--- | :--- | :--- |
| **变动成本率** | `已报告赔款/满期保费 + 费用金额/签单保费` | **成本控制能力**。诊断成本端的整体表现。 |
| **满期率** | `满期保费 / 签单保费` | 反映当期保费中已实现风险价值的部分。 |
| **满期出险率** | `(赔案件数/保单件数)/(满期保费/签单保费)` | 衡量已满期保单的出险频率。 |
...
| **单均费用** | `费用金额/保单件数` | **运营效率**。衡量平均每张保单的费用成本。 |
